...
- **Sync across devices with a link.** In **My Words → Add / Edit Words**, tap
  **🔗 Share my words**. On a phone/tablet this opens the normal share sheet
  ...
  are skipped). There's also a **QR code** 📷 — just point another device's
  camera at it to open the link (great for tablet → phone, no typing). The words
...
| `qrcode.js`  | QR-code generator for share links (vendored, MIT) |

## Credits

- QR codes are generated by [`qrcode-generator`](https://github.com/kazuhikoarase/qrcode-generator)
  by Kazuhiko Arase (MIT license), bundled locally so the app stays
